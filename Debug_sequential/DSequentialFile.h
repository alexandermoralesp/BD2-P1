#ifndef DSEQUENTIALFILE_H
#define DSEQUENTIALFILE_H

#include <string>
#include <fstream>
#include <vector>
#include <algorithm>
#include "DRegistros.h"
#include <ctgmath>
#include <cstdio>
...
public:
    void reorganize(int& accesos);
    SequentialFile(std::string _base_path);
    void add(Record reg, int& accesos);
    template <typename Key_t>
    std::vector<Record> rangeSearch(Key_t begin_key, Key_t end_key, int& accesos);
    template <typename Key_t>
    void remove(Key_t key, int& accesos);
    template <typename Key_t>
    Record search(Key_t key, int& accesos);
    void loadAll(std::vector<Record> &regs, std::vector<NextLabel> &labs);
};

template <typename Record>
template <typename Key_t>
void SequentialFile<Record>::remove(Key_t key,int& accesos)
{
    ...
    {
        int m = (l + r) / 2;
        dataFile.seekg(sizeof(NextLabel) + m * row_sizeof, std::ios::beg);accesos++;
        dataFile >> RowMatch;accesos++;
    ...
    if (!(l <= r) || RowPtr.nextRow == 0)
    {
        ...
        auxFile.seekg(0, std::ios::beg);accesos++;
    ...
    dataFile.seekg(0, std::ios::beg);accesos++;
    dataFile >> PreviousRowPtrReader;accesos++;
    if (PreviousRowPtrReader == PreviousRowPtr)
    {
        dataFile.seekp(0, std::ios::beg);accesos++;
        dataFile << RowPtr;accesos++;
    }
    else
    {
        ...
        l = 0;
        dataFile.seekg(0, std::ios::end);accesos++;
        r = (dataFile.tellg() - row_sizeof - sizeof(NextLabel)) / row_sizeof;

        while (l <= r)
        {
            int m = (l + r) / 2;
            dataFile.seekg(m * row_sizeof, std::ios::beg);accesos++;
            dataFile >> PreviousRowPtrReader;accesos++;
            if (PreviousRowPtrReader == PreviousRowPtr)
            {
                dataFile.seekp(m * row_sizeof, std::ios::beg);accesos++;
                dataFile << RowPtr;accesos++;
                break;
            }
        ...
        auxFile.seekg(0, std::ios::end);accesos++;
        r = auxFile.tellg() / row_sizeof;
        for (l = 0; l < r, PreviousRowPtrReader != PreviousRowPtr; r++)
        {
            auxFile.seekg(row_sizeof * l - sizeof(Record), std::ios::beg);accesos++;
            auxFile >> PreviousRowPtrReader;accesos++;
            if (PreviousRowPtrReader == PreviousRowPtr)
            {
                auxFile.seekp(row_sizeof * l - sizeof(Record), std::ios::beg);accesos++;
                auxFile << RowPtr;accesos++;
                break;
            }
        }
    ...
    {
        dataFile.seekp(row_sizeof * PreviousRowPtr.nextRow, std::ios::beg);accesos++;
        PreviousRowPtr.nextRow = 0;
        dataFile << PreviousRowPtr;accesos++;
        sizeData--;
    }
    else
    {
        auxFile.seekp((PreviousRowPtr.nextRow - 1) * row_sizeof + sizeof(Record), std::ios::beg);accesos++;
        PreviousRowPtr.nextRow = 0;
        auxFile << PreviousRowPtr;accesos++;
        sizeAux--;
    }
    eliminados++;
...
template <typename Record>
void SequentialFile<Record>::reorganize(int &accesos)
{
    sizeData += sizeAux;
    sizeAux = 0;
    K_max_aux = static_cast<int>(log2(sizeData)) + 1;

    std::fstream fileData(base_path + BinSuffix, std::ios::in | std::ios::binary);accesos++;
    std::fstream fileAux(base_path + AuxSuffix, std::ios::in | std::ios::binary);accesos++;

    std::string copyFileName = "tCopy.dat";
    std::fstream tempCopyWrite(copyFileName, std::ios::out | std::ios::app | std::ios::binary);accesos++;

    NextLabel printPtr;
    Record printRecord;
    int marker = 1;
    fileData >> printPtr;accesos++;
    while (printPtr.nextRow != -1)
    {
        tempCopyWrite <<NextLabel{marker++, 'd'};accesos++;
        if (printPtr.nextRowFile == 'd'){
            fileData >> printRecord >> printPtr;accesos++;}
        else{
            fileAux >> printRecord >> printPtr;accesos++;}
        tempCopyWrite << printRecord;accesos++;
    }
    tempCopyWrite << printPtr;accesos++;

    tempCopyWrite.close();
    fileData.close();
    fileAux.close();

    fileData.open(base_path + BinSuffix, std::ios::trunc | std::ios::out);accesos++;
    fileData.close();

    tempCopyWrite.open(copyFileName, std::ios::in | std::ios::binary);accesos++;
    ...
    {
        fileData << printPtr;accesos++;
        tempCopyWrite >> printRecord >> printPtr;accesos++;
        fileData << printRecord;accesos++;
    }
    fileData << printPtr;accesos++;
    ...
    fileAux.open(base_path + AuxSuffix, std::ios::trunc | std::ios::out);accesos++;
    fileAux.close();
}
...
template <typename Record>
template <typename Key_t>
std::vector<Record> SequentialFile<Record>::rangeSearch(Key_t begin_key, Key_t end_key, int &accesos)
{
    if (begin_key == end_key)
    {
        std::vector<Record> result;
        result.push_back(search(begin_key, accesos));
        return result;
    }
    ...
    if (key_group[0] > key_group[1])
        ...
        return std::vector<Record>{search<Key_t>(begin_key, accesos)};

    Record row_group[2];
    NextLabel label_group[2];

    std::ifstream fileData(base_path + BinSuffix, std::ios::in | std::ios::binary);accesos++;
    std::ifstream fileAux(base_path + AuxSuffix, std::ios::in | std::ios::binary);accesos++;
    for (int i = 0; i < 2; i++)
    {
        int l = 0;
        fileData.seekg(0, std::ios::end);accesos++;
        int r = (fileData.tellg() - row_sizeof - sizeof(NextLabel)) / row_sizeof;
        while (l <= r)
        {
            int m = (l + r) / 2;
            fileData.seekg(sizeof(NextLabel) + m * row_sizeof, std::ios::beg);accesos++;
            fileData >> row_group[i];accesos++;
            ...
            {
                fileData >> label_group[i];accesos++;
                break;
            }
        }
        if (key_group[i] != row_group[i] || label_group[i].nextRow == 0)
        {
            ...
            while (fileAux >> row_group[i])accesos++;
            {
                if (row_group[i] == key_group[i])
                {
                    fileAux >> label_group[i];accesos++;
                    if (label_group[i].nextRow != 0)
                        break;
                }
                else
                    fileAux.ignore(sizeof(NextLabel));accesos++;
            }
    ...
    while (!(label_group[0] == label_group[1]))
    {
        ...
        {
            fileData.seekg((label_group[0].nextRow - 1) * row_sizeof + sizeof(NextLabel), std::ios::beg);accesos++;
            fileData >> row_group[0] >> label_group[0];accesos++;
        }
        else
        {
            fileAux.seekg((label_group[0].nextRow - 1) * row_sizeof, std::ios::beg);accesos++;
            fileAux >> row_group[0] >> label_group[0];accesos++;
        }
...
template <typename Record>
template <typename Key_t>
Record SequentialFile<Record>::search(Key_t key, int& accesos)
{
    std::ifstream file(base_path + BinSuffix, std::ios::in | std::ios::binary);accesos++;

    file.seekg(0, std::ios::end);accesos++;
    int l = 0;
    ...
    {
        int m = (l + r) / 2;
        file.seekg(sizeof(NextLabel) + m * row_sizeof, std::ios::beg);accesos++;
        file >> result;accesos++;
        if (objective > result)
            l = m + 1;
    ...
    file.open(base_path + AuxSuffix, std::ios::in | std::ios::binary);accesos++;
    ...
    while (file >> result)
    {
        ...
        file.ignore(sizeof(NextLabel));accesos++;
    }
...
template <typename Record>
void SequentialFile<Record>::add(Record reg, int &accesos)
{
    if (sizeAux >= K_max_aux)
        reorganize(accesos);

    std::fstream fileAux(base_path + AuxSuffix, std::ios::in | std::ios::out | std::ios::binary);accesos++;
    std::fstream fileData(base_path + BinSuffix, std::ios::in | std::ios::out | std::ios::binary);accesos++;
    ...
    if (r == -1)
    {
        fileData.seekg(0, std::ios::beg);accesos++;
        fileData >> prev_ptr;accesos++;
    }
    else
    {
        fileData.seekg(l * row_sizeof - sizeof(Record), std::ios::beg);
        fileData >> prev >> prev_ptr;accesos++;accesos++;
        prev_ptr_offset = fileData.tellg() - sizeof(NextLabel);accesos++;
    }

    Record aux_prev;
    fileAux.seekg(0, std::ios::beg);accesos++;
    ...
    fileAux.close();
    fileAux.open(base_path + AuxSuffix, std::ios::out | std::ios::binary | std::ios::app);accesos++;
    fileAux.seekp(0, std::ios::end);accesos++;
    fileAux << reg << prev_ptr;accesos++;accesos++;
    fileAux.close();

    fileAux.open(base_path + AuxSuffix, std::ios::in | std::ios::out | std::ios::binary);accesos++;
    fileAux.seekg(0, std::ios::end);accesos++;
    prev_ptr = {((int)fileAux.tellp()) / row_sizeof, 'a'};

    if (!prev_in_aux)
    {
        if (r == -1)
            {fileData.seekp(0, std::ios::beg);accesos++;}
        else
            {fileData.seekp(prev_ptr_offset, std::ios::beg);accesos++;}
        fileData << prev_ptr;accesos++;
    }
    else
    {
        fileAux.seekp(prev_ptr_offset, std::ios::beg);accesos++;
        fileAux << prev_ptr;accesos++;
    }
