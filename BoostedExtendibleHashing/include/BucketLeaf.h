...
void BucketLeaf<T>::add_element(T key, long value) {
  if (size_ == FACTOR_BLOCK) throw std::runtime_error("BucketLeaf is full");
...
void BucketLeaf<T>::remove_element(T key) {
  if (size_ == 0) throw std::runtime_error("BucketLeaf is empty");
  if (elements_->find(key) != elements_->end()) {
    elements_->erase(key);
    size_--;
  }
...
void BucketLeaf<T>::read_from_file(std::string filename, long position) {
  ...
  if (!file.is_open())
    throw std::runtime_error(
        "[ERROR]: File not found from BucketLeaf::read_from_file");
  file.seekg(position * get_sizeof(), std::ios::beg);
...
void BucketLeaf<T>::write_to_file(std::string filename, long position) {
  ...
  if (!file.is_open())
    throw std::runtime_error(
        "[ERROR]: File not found from BucketLeaf::write_to_file");
  file.seekp(position * get_sizeof(), std::ios::beg);
...
int BucketLeaf<T>::get_sizeof() {
  if (std::is_same<T, std::string>::value) {
    return sizeof(int) + sizeof(long) +
           (MAX_LENGTH_VARCHAR + sizeof(long)) * FACTOR_BLOCK;
  }
  return sizeof(long) + sizeof(int) + (sizeof(long) + sizeof(T)) * FACTOR_BLOCK;
}
